...
export function generateErrorSdk(): string {
  ...
  var TSA={
    ...
    captureMessage:function(msg,level){send({message:String(msg),type:'Message',level:level||'info',url:location.href,environment:cfg.environment,release:cfg.release,tags:merge(scope.tags,{}),user:scope.user,extra:merge(scope.extra,{}),sdkVersion:'tsa-sdk/1'});},
