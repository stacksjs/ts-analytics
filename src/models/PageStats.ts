...
const PageStatsModel: Model = {
  ...
  dynamodb: {
    ...
    sk: (stats: { period: string, periodStart: string, path: string }): string =>
      `PAGESTATS#${stats.period.toUpperCase()}#${stats.periodStart}#${encodeURIComponent(stats.path)}`,
    gsi1pk: (stats: { siteId: string, period: string, periodStart: string }): string =>
      `SITE#${stats.siteId}#PAGESTATS#${stats.period.toUpperCase()}#${stats.periodStart}`,
    gsi1sk: (stats: { pageViews: number, path: string }): string =>
